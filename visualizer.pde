...
void setup(){
  ...
  player = minim.loadFile("data/track1.mp3", 1024);
  beat = new BeatDetect();
...
void draw() {
  ...
  noFill();
  translate(luapos, -600, -1600); //muda a posicao da lua, o terceiro argumento e o z
  ...
  if (luapos > 1000) { //o ngc do cont2, se a pos dele for 1050 ele comeca a ir pra esquerda, e qnd for -600 vai pra direita
    cont2 = true;
    println(luaposcord);
  } 
  if (luapos<-500) {
  ...
  luaposcord = map(luapos, -600, 1100, 56, 440);
  popMatrix();
...
void mouseClicked() {
  ...
  if (mouseX >= luaposcord - 30 && mouseX <= luaposcord && mouseY >= 40 && mouseY <= 95 || mouseX <= luaposcord + 20 && mouseX >= luaposcord - 30 && mouseY >= 40 && mouseY <= 95) {
    click++;
    temacor();
  }
}
